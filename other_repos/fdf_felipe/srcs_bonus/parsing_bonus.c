...
// static void	print_img(t_data *data);
...
static void	print_map(t_data *data)
{
	...
	while (y < data->map.height)
	{
		while (x < data->map.width)
		{
			printf(" x%d,y%d,z%d",//,c%d ",
			 data->map.points[i].x, data->map.points[i].y, data->map.points[i].z);//, data->points[i].color);
			x++;
			i++;
			if (x == data->map.width)
				printf("\n");
	...
	printf("total nbr of coordinates printed -> %d\n", i);
}
...
// 		{
// 			// printf(" x%d,y%d ",
// 			//  data->map.points[i].display[0], data->points[i].display[1]);
// 			x++;
// 			i++;
// 			if (x == data->map.width)
// 				printf("\n");
